 ...
 * @returns {any[]}
 */
Array.prototype.reduceMap = function (callback, thisOrg) {
  return this.reduce((pre, cur, index, arr) => {
    cur = callback.call(thisOrg, cur, index, arr)
    pre.push(cur)
    return pre
  }, [])
}
 ...
 * @returns {any[]}
 */
Array.prototype.reduceFilter = function (callback, thisOrg) {
  return this.reduce(
    (pre, cur, index, arr) => {
      return pre.concat(
        callback.call(thisOrg, cur, index, arr) ? cur : []
      )
    },
    []
  )
