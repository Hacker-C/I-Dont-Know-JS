export default {
  '/JS/': [
    {
      items: [
        { text: '镇站之图', link: '/JS/thanks-for-inventing-js.md' },
        { text: '本书介绍', link: '/JS/intro.md' }
      ]
    },
    {
      text: '🪄 奇技淫巧',
      collapsible: true,
      collapsed: true,
      items: [
        { text: '01. == 与 === 问题', link: '/JS/uncanny-tricks/ch01' },
        { text: '02. foo[1][2][3] 累加器', link: '/JS/uncanny-tricks/ch02' },
        { text: '03. “数组” 负数取值', link: '/JS/uncanny-tricks/ch03' },
        { text: '04. (1).add(2).add(3)', link: '/JS/uncanny-tricks/ch04' },
        { text: '05. 我真不会 parseInt', link: '/JS/uncanny-tricks/ch05' },
        { text: '06. name 的长度是多少', link: '/JS/uncanny-tricks/ch06' },
        { text: '07. 快速产生连续字符序列', link: '/JS/uncanny-tricks/ch07' },
        { text: '08. [...10]', link: '/JS/uncanny-tricks/ch08' },
        { text: '09. 连续正则匹配结果不同', link: '/JS/uncanny-tricks/ch09' },
        { text: '10. 二维数组初始化问题', link: '/JS/uncanny-tricks/ch10' },
        { text: '11. 巧用 JS 中的运算符', link: '/JS/uncanny-tricks/ch11' },
        { text: '12. 万物皆可 reduce', link: '/JS/uncanny-tricks/ch12' },
        { text: '13. [].every(item => false)', link: '/JS/uncanny-tricks/ch13' },
        { text: '14. isNaN 和 Number.isNaN', link: '/JS/uncanny-tricks/ch14' },
        { text: '15. 标签函数与模板字符串', link: '/JS/uncanny-tricks/ch15' },
        { text: '16. 只执行一次的函数', link: '/JS/uncanny-tricks/ch16' },
        { text: '17. Node.contains()', link: '/JS/uncanny-tricks/ch17' },
        { text: '18. 事件总线 EventBus', link: '/JS/uncanny-tricks/ch18' },
        { text: '19. AOP 在 JS 中的实现', link: '/JS/uncanny-tricks/ch19' },
        { text: '20. void 运算符', link: '/JS/uncanny-tricks/ch20' },
        { text: '21. 使对象具有可迭代性', link: '/JS/uncanny-tricks/ch21' },
        { text: '22. 柯里化与偏函数', link: '/JS/uncanny-tricks/ch22' },
        { text: '23. 多维数组扁平化', link: '/JS/uncanny-tricks/ch23' },
        { text: '24. 树形与扁平数据结构转换', link: '/JS/uncanny-tricks/ch24' },
        { text: '25. 深入类型判断', link: '/JS/uncanny-tricks/ch25' },
        { text: '26. Symbol 内置属性的妙用', link: '/JS/uncanny-tricks/ch26' },
        { text: '27. if-else 代码优化方案', link: '/JS/uncanny-tricks/ch27' },
        { text: '28. URLSearchParams 的妙用', link: '/JS/uncanny-tricks/ch28' }
      ]
    },
    {
      text: '📘 日常随记',
      collapsible: true,
      collapsed: true,
      items: [
        {
          text: '01. JavaScript',
          link: '/JS/daily-notes/JavaScript'
        },
        {
          text: '02. CSS',
          link: '/JS/daily-notes/CSS'
        },
        {
          text: '03. 二分搜索篇',
          link: '/JS/daily-notes/ch01'
        },
        {
          text: '04. 快速幂模板',
          link: '/JS/daily-notes/ch02'
        },
        {
          text: '05. 二叉树遍历模板',
          link: '/JS/daily-notes/ch03'
        },
        {
          text: '06. 随机打乱算法',
          link: '/JS/daily-notes/ch04'
        }
      ]
    }
  ]
}
